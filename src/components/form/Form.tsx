...
function Form() {
  ...
  const calculateMaterials = () => {
    ...
    const [rollWidth, rollHeight] = rollSize.split("x").map(parseFloat || 0);
    const stripsPerRoll = Math.floor(rollHeight / (roomHeight + parseFloat(rapport)));
    const stripsNeeded = Math.ceil(wallpaperArea / roomHeight / rollWidth);
